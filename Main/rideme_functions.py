        ...
        try:
            with open(filename, 'r') as file:
                ...
                for row in reader:
                    ...
                    vehicle_id = f"{row['Plate']}-{row['BookingID']}"

                    try:
                        vehicle = Vehicle.from_type(vehicle_type, vehicle_id)
                    except ValueError as e:
                        print(e)
                        continue  # skip this booking if vehicle type is unrecognized
    ...
    def save_user_booking(self, booking):
        ...
        try:
            with open(filepath, 'a', newline='') as file:
                writer = csv.writer(file)
                if write_header:
                    writer.writerow(["BookingID", "Time", "User", "Driver", "Plate", "Start", "End","Vehicle", "Rate/km", "Distance", "Cost", "Tax", "Total", "Status", "Rating"])
                writer.writerow([
                    booking.booking_id,
